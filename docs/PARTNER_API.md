...
| 방식 | 설명 | Wirex가 보는 주체 |
|------|------|-------------------|
| API | 파트너가 자체 앱에서 `/api/partner/v1/*` 호출 | ICOCARD |
| 서브 솔루션 | 브랜드 회원앱 `/s/{slug}` — ICOCARD 키 | ICOCARD |
| 서브 솔루션 단독형 | 운영 전용. 업체의 Wirex 계약 키. 재판매 키 없음 | 해당 업체 |

지갑 3방식은 가맹점마다 **본사설정 따름** 또는 **별도**로 잠급니다. 별도면 임베디드/외부 EOA/브리지를 직접 고릅니다.

관리자에서 업체 등록 시 **배포 방식**과 지갑 정책(본사설정 따름/별도)을 고릅니다. API·서브 솔루션은 키트(MID/API Key/Secret/HMAC)를 1회 표시합니다. **서브 솔루션 단독형**은 ICOCARD 재판매 키를 발급하지 않으며 회원앱 `/s/{slug}` 운영만 합니다.
