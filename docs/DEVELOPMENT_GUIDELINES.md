...
| **솔루션 패키지** | 타 업체 판매(화이트라벨) 가능하도록 브랜드/도메인/테넌트 분리. [SOLUTION_PACKAGING.md](./SOLUTION_PACKAGING.md) |
| **Wirex 실연동 시점** | Sandbox 전용 API Key 수령 후에만 Mock→Real 전환 |

### 패키지/화이트라벨 규칙

- 상품명·운영사명·로고 → `brandStore` / Admin Brand (하드코딩 금지)
- 파트너 데이터 → 항상 `partnerId` 스코프
- Wirex credentials → **배포(인스턴스) 단위**. 구매사별 키 혼용 금지
- 새 기능 추가 시 “다른 브랜드로 납품해도 동작하는가?”를 확인
