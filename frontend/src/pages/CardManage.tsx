...
export default function CardManage() {
  ...
  const handleWallet = async (c: Card, wallet: 'apple_pay' | 'google_pay') => {
    try {
      ...
      alert(wallet === 'apple_pay' ? t('cards.applePayDone') : t('cards.googlePayDone'));
    } catch (e) {
      alert((e as Error).message);
    }
  };
  ...
  return (
    <div className="app-container">
      <div className="page-header">
        <h1 className="page-title">{t('cards.manageTitle')}</h1>
      </div>
      ...
      {limitModal && (
        <div className="modal-overlay" onClick={() => setLimitModal(null)}>
          <div className="modal-content card-surface" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="muted-text modal-desc">•••• {limitModal.panLast4}</p>
            <input
              type="number"
              value={newLimit}
